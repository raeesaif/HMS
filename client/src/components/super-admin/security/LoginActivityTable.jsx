...
export function LoginActivityTable({ activity }) {
  return (
          ...
          <Table className="min-w-[650px]">
            <TableHeader className="bg-slate-50 [&_tr]:border-b-0">
              <TableRow className="hover:bg-transparent">
                {['Date', 'Time', 'Device', 'IP Address', 'Location', 'Status'].map((label) => (
            ...
            <TableBody>
              {activity.map((entry, index) => (
                <TableRow key={entry.id} className={`border-b-0 ${index % 2 ? 'bg-slate-50/70' : 'bg-white'}`}>
                  ...
                  <TableCell className="px-4 py-3 text-slate-600">{entry.location}</TableCell>
