...
export function SessionsTable({ sessions, onSignOut, onSignOutAll }) {
    ...
    <Card className="rounded-xl border-border shadow-sm">
      ...
      <CardContent className="space-y-2 pt-3">
        {sessions.map((session) => (
          <div key={session.id} className="flex flex-col gap-3 rounded-lg border border-slate-200 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="mt-0.5 flex items-center gap-1 text-xs text-slate-500">
                  ...
                  {session.browser} · {session.os} · {session.ip} · {session.location} · {session.lastActive}
                </p>
              </div>
            </div>
            {!session.isCurrent && (
