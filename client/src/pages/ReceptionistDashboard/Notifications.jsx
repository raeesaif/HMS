...
import { NotificationsList } from '@/components/reception/notifications/NotificationsList';
import { NotificationDetailsDialog } from '@/components/dialogs/receptionist/NotificationDetailsDialog';
import { ConfirmDialog } from '@/components/dialogs/ConfirmDialog';
import { useNotifications } from '@/hooks/useNotifications';
import { markNotificationAsRead, markAllNotificationsAsRead } from '@/services/notificationService';
import { notificationTypeOptions } from '@/data/receptionistNotifications';

const PAGE_SIZE = 6;

const Notifications = () => {
  const navigate = useNavigate();
  const { notifications, setNotifications, isLoading, error, reload } = useNotifications();
    ...
    <div className="mx-auto w-full max-w-[1600px] space-y-6">
      <section className="flex flex-col justify-between gap-4 lg:flex-row lg:items-center">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">Stay updated on appointments, check-ins, queue, beds, and billing activity.</p>
        </div>
